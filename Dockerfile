...
ARG VITE_AD_BREAK_MINUTES=20
ARG VITE_AD_BREAK_MAX_PER_HOUR=2
RUN printf "VITE_PB_URL=%s\nVITE_ADS_ENABLED=%s\nVITE_GAM_AD_UNIT_CHAT=%s\nVITE_GAM_AD_UNIT_INTERSTITIAL=%s\nVITE_AD_BREAK_MINUTES=%s\nVITE_AD_BREAK_MAX_PER_HOUR=%s\n" \
  "$VITE_PB_URL" "$VITE_ADS_ENABLED" "$VITE_GAM_AD_UNIT_CHAT" "$VITE_GAM_AD_UNIT_INTERSTITIAL" "$VITE_AD_BREAK_MINUTES" "$VITE_AD_BREAK_MAX_PER_HOUR" > .env
RUN npm run build
